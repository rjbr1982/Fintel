      ...
      children: [
        ...
        SizedBox(
          ...
          child: OutlinedButton.icon(
            style: OutlinedButton.styleFrom(foregroundColor: Colors.black87, side: const BorderSide(color: Colors.black87, width: 1.5), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10))), 
            icon: const Icon(Icons.logout, size: 18), 
            label: const Text('התנתקות מהחשבון', style: TextStyle(fontWeight: FontWeight.bold)), 
            onPressed: () async { Navigator.pop(context); try { await GoogleSignIn().disconnect(); } catch (_) {} try { await GoogleSignIn().signOut(); } catch (_) {} await FirebaseAuth.instance.signOut(); if (context.mounted) Navigator.of(context).pushNamedAndRemoveUntil('/', (Route<dynamic> route) => false); }
          )
        ),
        const SizedBox(height: 8),
        SizedBox(
          ...
          child: TextButton.icon(
            ...
            label: const Text('מחיקת חשבון לצמיתות', style: TextStyle(fontWeight: FontWeight.bold)), 
            onPressed: () {
...
void _showDeleteAccountConfirm(BuildContext context) {
  showDialog(
    ...
    builder: (ctx) => AlertDialog(
      ...
      title: const Text('⚠️ מחיקת חשבון', style: TextStyle(color: Colors.red, fontWeight: FontWeight.bold)),
      content: const Text('פעולה זו תמחק לצמיתות את חשבונך ואת כל הנתונים הפיננסיים שלך משרתינו. לא ניתן לבטל פעולה זו. האם להמשיך?', style: TextStyle(color: Colors.black87)),
      actions: [
        TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('ביטול', style: TextStyle(color: Colors.blueGrey))), 
        ElevatedButton(
          ...
          onPressed: () async { 
            ...
            try {
              ...
              if (context.mounted) {
                ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('שגיאת אבטחה: יש להתנתק ולהתחבר מחדש לפני מחיקת חשבון לצמיתות.'), backgroundColor: Colors.red, duration: Duration(seconds: 4)));
              }
            }
          }, 
          child: const Text('מחק לצמיתות', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold))
        )
      ],
    ),
  );
